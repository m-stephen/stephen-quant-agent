...
Machine-local market-data paths belong in ignored `configs/*.local.json` files. The repository
tracks only `configs/qd-paths.example.json`; CLI values override local configuration values.
Raw market data, generated reports, registries, credentials, and absolute terminal paths must
never be committed.
