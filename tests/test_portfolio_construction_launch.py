...
import pytest
from test_portfolio_construction_runtime import synthetic_spec

from stephen_quant.discovery import portfolio_construction_launch as m
...
def test_all_four_reserved_before_child_and_failed_debt_cannot_replay(
    ...
    assert terminal["raw_global_trial_lower_bound"] == 3733
...
def test_actual_api_comment_requires_own_version_issue_hash_and_time(tmp_path, monkeypatch):
    obj = {
        ...
        "body": "V11.22-PREREGISTRATION-SHA256: " + "d" * 64,
    ...
    for key, value in (
        ...
        ("issue_url", m.ISSUE_URL.replace("184", "185")),
...
def test_prepare_binds_parent_source_code_and_driver_without_claim(tmp_path, monkeypatch):
    ...
    sources, calendar = {"daily": "a" * 64}, ["2022-01-04", "2023-01-03", "2024-12-31"]
    parent = {
        "source_evidence": sources,
        "calendar": calendar,
        "prior_debt": 3729,
        "inherited": {
            "root": str((tmp_path / "old").resolve()),
            "trial_ids": {"lowvol-82": "synthetic"},
            "files": {"registry.sqlite3": "b" * 64, "history/history.json": "c" * 64},
        },
    }
    ...
    monkeypatch.setattr(m, "completed_parent_evidence", lambda _: parent)
